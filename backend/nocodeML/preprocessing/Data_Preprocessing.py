...
def missing_value(data, strategy='mean') :
    # Strategies - mean, median, most frequent(mode) or remove.
    ...
    if strategy == 'remove' :
        ...
        for i in range( len( data.columns ) ) :
            ...
            try :
                if strategy == 'mode' or (type( data.iloc[0, i] ) == str) :
                    data[data.columns[i]] = data[data.columns[i]].fillna( stat.mode( data[data.columns[i]] ) )
            except :
                c = Counter( list( data[data.columns[i]] ) )
                mode = c.most_common( 1 )[0]
                data[data.columns[i]] = data[data.columns[i]].fillna( mode )
...
def feature_scaling(data) :
    for i in range( len( data.columns ) ) :
        if (type( data.iloc[0, i] ) == int or type( data.iloc[0, i] ) == float or type(
                data.iloc[0, i] ) == np.float64 or type( data.iloc[0, i] ) == np.int64) :
            temp = []
            mean = np.mean( data[i] )
            std = np.std( data[i] )
            for j in data[i] :
